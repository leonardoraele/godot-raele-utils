...
public partial class CallMethod : VariantSource
{
	...
	[Export(PropertyHint.NodePathValidTypes, nameof(Node))] public NodePath Target = "";
	[Export] public string Method
		{ get; set { field = value; this.NotifyPropertyListChanged(); } }
		= "";
	[Export] public Godot.Collections.Array MethodArguments = [];
	...
	public Node? TargetNode
		=> this.GetLocalScene()?.GetNodeOrNull(this.Target);
	public GodotMethodInfo? MethodInfo
		=> this.TargetNode?.GetMethodList()
			.Select(GodotMethodInfo.FromDictionary)
			.FirstOrDefault(method => method.Name == this.Method);
	...
	protected override Variant _GetValue()
		=> this.TargetNode?.Call(this.Method) ?? Variant.NULL;
